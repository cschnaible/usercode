...
models = ["ZPrimeQ"]#,"DY"]
#models = ['ZPrimeQ','ZPrimeSSM','ZPrimePSI','ZPrimeT3L','ZPrimeB-L']
#models = ['ZPrimeLR','ZPrimeR','ZPrimeY']
#decays = {"EE":11,"MuMu":13}
decays = {"EE":11}
#pdfs = ['NNPDF30nlo','CT10nlo','CT14nlo']#,'MMHT2014nlo68cl']
        ...
        for i in range(0,len(massBins)-1):
            for model in models:
                for decay, decayN in decays.iteritems():
                    ...
                    if model == "DY":
                        if massBins[i+1] == -1:
                            f = open("%sTo%s_M%dToInf_13TeV-pythia8_%s_cff.py"%(model,decay,massBins[i],pdftouse),"w")
                        else:
                            f = open("%sTo%s_M%dTo%d_13TeV-pythia8_%s_cff.py"%(model,decay,massBins[i],massBins[i+1],pdftouse),"w")
                    else:
                        if massBins[i+1] == -1:
                            f = open("%sTo%s_ResM%d_M%dToInf_Interference_13TeV-pythia8_%s_cff.py"%(model,decay,mass,massBins[i],pdftouse),"w")
                        else:
                            f = open("%sTo%s_ResM%d_M%dTo%d_Interference_13TeV-pythia8_%s_cff.py"%(model,decay,mass,massBins[i],massBins[i+1],pdftouse),"w")
                    f.write(fragment)
                    f.close()
                    #exit()
    ...
    for mass in masses:
        for model in models:
            for decay, decayN in decays.iteritems():
                ...
                params["PDFSET"] = PDFSETS[pdftouse]
                fragment = template%params
                f = open("%sTo%s_ResM%d_13TeV-pythia8_%s_cff.py"%(model,decay,mass,pdftouse),"w")
                f.write(fragment)
                f.close()
